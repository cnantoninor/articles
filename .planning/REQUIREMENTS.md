...
- [ ] **UNDR-01**: Bounded contexts with glob-activated rules example (`.cursor/rules/*.mdc`)
- [ ] **UNDR-02**: Ubiquitous language via `constants.py` example
- [ ] **UNDR-03**: Business rules loaded in context files example (`experiments.mdc`)

### Reliability Vertex (TDD Pull)

- [ ] **RELY-01**: Human-authored E2E test as circular validation breaker (`test_user_journey_e2e.py`)
- [ ] **RELY-02**: Mock boundaries defined in testing rules
- [ ] **RELY-03**: "Do NOT Over-Generate Tests" rule

### Speed Vertex (Workflow Pull)

- [ ] **SPEED-01**: Workflow commands for structured LLM interactions (`.cursor/commands/`)
- [ ] **SPEED-02**: LOC metrics in PR descriptions ("Pure Code Added" column)
- [ ] **SPEED-03**: Verification-before-fix workflow (`verifyeventuallyfix.md`)

### Meta/Workflow

- [ ] **META-01**: GSD as successor to custom commands
- [ ] **META-02**: Multi-AI context sync script (`sync-ai-instructions.sh`)
