#!/usr/bin/env bash
# sync-rules.sh — Create/update symlinks for AI rules across Cursor and Claude Code
#
# Source of truth: .ai/rules/*.md
# Targets:
#   .cursor/rules/*.mdc  (symlinks with .mdc extension for Cursor)
#   .claude/rules/*.md   (symlinks with .md extension for Claude Code)
#   .cursorrules         (symlink to .ai/context.md — always-applied)
...
for dir in "$CURSOR_RULES_DIR" "$CLAUDE_RULES_DIR"; do
    if [ ! -d "$dir" ]; then
        mkdir -p "$dir"
        ok "Created $dir"
    else
...
for root_link in ".cursorrules" "CLAUDE.md"; do
...
verify_count=0
for link in "$CURSOR_RULES_DIR"/*.mdc "$CLAUDE_RULES_DIR"/*.md; do
    [ -L "$link" ] || continue
    if [ -e "$link" ]; then
...
for root_link in "$REPO_ROOT/.cursorrules" "$REPO_ROOT/CLAUDE.md"; do
